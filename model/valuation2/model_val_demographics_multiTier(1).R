...
for (j in 1:(nyear - 1)){
  ...
  ls_demo[[tierName]]$temp <- list(
    ...
    active2defrRet.vest   = with(ls_demo[[tierName]], wf_active[, , j] * p_active2defrRet.vest),     # This will join wf_term[, , j + 1, j + 1], note that workers who terminate in year j won't join the terminated group until j+1. 
    active2defrRet.refund = with(ls_demo[[tierName]], wf_active[, , j] * p_active2defrRet.refund), 
    ...
    defrRet.vest2dead    = with(ls_demo[[tierName]], wf_defrRet.vest[, , j, ]   * (p_defrRet.vest2dead   %>% filter(year == j + init_year - 1))[["qxm.defrRet.vest"]]), 
    defrRet.refund2dead  = with(ls_demo[[tierName]], wf_defrRet.refund[, , j, ] * (p_defrRet.refund2dead %>% filter(year == j + init_year - 1))[["qxm.defrRet.refund"]]), 
  ...
  ls_demo[[tierName]]$temp$out_defrRet.vest <- ls_demo[[tierName]]$temp$defrRet.vest2dead    # This is a 3D array 
  ls_demo[[tierName]]$temp$in_defrRet.vest  <- ls_demo[[tierName]]$temp$active2defrRet.vest  # This is a matrix
  
  ls_demo[[tierName]]$temp$out_defrRet.refund <- ls_demo[[tierName]]$temp$defrRet.refund2dead    # This is a 3D array 
  ls_demo[[tierName]]$temp$in_defrRet.refund  <- ls_demo[[tierName]]$temp$active2defrRet.refund  # This is a matrix
  ...
  ls_demo[[tierName]]$temp$in_dead <- 
    ...
    apply(ls_demo[[tierName]]$temp$defrRet.vest2dead,    c(1,2), sum) +   # 
    apply(ls_demo[[tierName]]$temp$defrRet.refund2dead,  c(1,2), sum) +   # 
  ...
  for (tierName in names(ls_demo)){
    ls_demo[[tierName]]$temp$out_active <- with(ls_demo[[tierName]]$temp, active2defrRet.vest + active2defrRet.refund + active2disbRet + active2servRet.la + active2dead) 
  ...
  ls_demo[[tierName]]$wf_defrRet.vest[, , j + 1, ] <- 
    apply((ls_demo[[tierName]]$wf_defrRet.vest[, , j, ] - ls_demo[[tierName]]$temp$out_defrRet.vest), 3, function(x) x %*% A) %>% 
    array(wf_dim.defrRet.vest[-3])
  
  ls_demo[[tierName]]$wf_defrRet.vest[, , j + 1, j + 1] <- ls_demo[[tierName]]$temp$in_defrRet.vest %*% A     # Note that termination year j = 1 correponds to init_year - 1?
  
  
  # terminated with refund
  ls_demo[[tierName]]$wf_defrRet.refund[, , j + 1, ] <- 
    apply((ls_demo[[tierName]]$wf_defrRet.refund[, , j, ] - ls_demo[[tierName]]$temp$out_defrRet.refund), 3, function(x) x %*% A) %>% 
    array(wf_dim.defrRet.refund[-3])
  
  ls_demo[[tierName]]$wf_defrRet.refund[, , j + 1, j + 1] <- ls_demo[[tierName]]$temp$in_defrRet.refund %*% A     # Note that termination year j = 1 correponds to init_year - 1?
...
# terminated with vested benefits
ls_demo[[tierName]]$wf_defrRet.vest <- 
  data.frame(
    expand.grid(
      ea   = range_ea,
      age  = range_age,
    ...
    n_defrRet.vest = as.vector(ls_demo[[tierName]]$wf_defrRet.vest)
  ) %>%
  filter(age >= ea)


# terminated with refund
ls_demo[[tierName]]$wf_defrRet.refund <- 
  data.frame(
    expand.grid(
      ea   = range_ea,
      age  = range_age,
    ...
    n_defrRet.refund = as.vector(ls_demo[[tierName]]$wf_defrRet.refund)
  ) %>%
  filter(age >= ea)
