...
#*******************************************************************************
  ...
  indivLiab_[[tn]]$defrRet <- 
    left_join(pop_[[tn]]$wf_defrRet.vest %>% select(year, ea, age, year_defrRet = year_defrRet.vest,  n_defrRet.vest), 
  ...
  indivLiab_[[tn]]$defrRet_yearsum <-  
    indivLiab_[[tn]]$defrRet %>% 
    select(any_of(c("ea", "age", "year", "year_defrRet", "age_defrRet", "start_year")), n_defrRet.vest, B.defrRet, ALx.defrRet) %>% 
    mutate(across(
      c(B.defrRet, ALx.defrRet), ~ .x * n_defrRet.vest
    )) %>% 
    group_by(year) %>% 
    # year total for each variable
    summarise(across(c(n_defrRet.vest, B.defrRet, ALx.defrRet), ~ sum(.x, na.rm = TRUE)),
              .groups = "drop") %>% 
    filter(year <= init_year + nyear - 1) %>% 
    as.matrix
