...
df_qxm.post_tier <-  
  ...
  select(grp, age, 
         qxm.post, qxmd.post, qxmd.post.nonocc, qxmd.post.occ, 
...
df_qxm.post_proj_tier <-  
  ...
  select(grp, age, 
         qxm.post_proj, qxmd.post_proj, qxmd.post.nonocc_proj, qxmd.post.occ_proj,
...
decrements_improvement %<>% 
  ...
  arrange(age, year) %>% 
  # filter(age == 90) %>% 
  mutate(across(!c(year), ~ seq(first(.x), last(.x), length.out = n()))) %>% 
  mutate(across(!c(year), ~ .x / .x[year == min(year)])) %>% 
  rename_with(~ paste0("impr_", .x), !c(year, age))
