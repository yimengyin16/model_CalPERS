...
salScale_full <- 
  ...
  mutate(yos = age - ea) %>% 
  left_join(select(salScale_tier, ea, yos, salScale), by = c("ea", "yos")) %>%
...
salary_full <- 
  ...
  mutate(salary = na2zero(salary),
  	     sx = ifelse(start_year <= init_year, salary[year == init_year] * scale_cum, 
...
# x <- add_entrantsDist(tierData_miscAll)
# x$entrants_dist
