...
{df_simNames <- 
  ...
  "all2t_benCut1_lowERC",                "Reduce benefit factor by 50%",
  ...
  "all2t_benCut2_lowERC",                "Reduce benefit factor by 25%",
...
df_B <- 
left_join(
  df_val_baseline$indivLiab$misc_classic$servRet.la %>% 
  ...
  df_val_reduced$indivLiab$misc_classic$servRet.la %>% 
    filter( (start_year == 2018 - (fn_age1 - fn_ea) & ea == fn_ea & age_servRet ==  fn_ageRet)|
            (start_year == 2018 - (fn_age2 - fn_ea) & ea == fn_ea & age_servRet ==  fn_ageRet)|
            (year_servRet == 2018 & age_servRet == fn_age3) # retiree
           ) %>% 
...
ls_benCut1          <- get_impactMembers2(simName = "misc2t_benCut1_lowERC", valName_baseline = "misc2t_bf100_cola2")
ls_colaCut1         <- get_impactMembers2(simName = "misc2t_colaCut1_lowERC", valName_baseline = "misc2t_bf100_cola2")
ls_benCut1_colaCut1 <- get_impactMembers2(simName = "misc2t_benCut1_colaCut1_lowERC", valName_baseline  = "misc2t_bf100_cola2")


ls_benCut2          <- get_impactMembers2(simName = "misc2t_benCut2_lowERC", valName_baseline = "misc2t_bf100_cola2")
ls_colaCut2         <- get_impactMembers2(simName = "misc2t_colaCut2_lowERC", valName_baseline = "misc2t_bf100_cola2")
ls_benCut2_colaCut2 <- get_impactMembers2(simName = "misc2t_benCut2_colaCut2_lowERC", valName_baseline  = "misc2t_bf100_cola2")
...
{df_simNames2 <- 
  ...
  "misc2t_colaCut1_lowERC",               "No COLA below 100% funded (7% DR)",
  "misc2t_colaCut1_highERC",              "No COLA below 100% funded (7% DR)",
  
  "misc2t_benCut1_colaCut1_lowERC",       "Reduce benefit factor by 50% and \nNo COLA below 100% funded (7% DR)",
  "misc2t_benCut1_colaCut1_highERC",      "Reduce benefit factor by 50% and \nNo COLA below 100% funded (7% DR)",
  
  "misc2t_benCut2_lowERC",                "Reduce benefit factor by 25%",
  "misc2t_benCut2_highERC",               "Reduce benefit factor by 25%",
  
  "misc2t_colaCut2_lowERC",               "Half COLA below 100% funded (7% DR)",
  "misc2t_colaCut2_highERC",              "Half COLA below 100% funded (7% DR)",
  
  "misc2t_benCut2_colaCut2_lowERC",       "Reduce benefit factor by 25% and \nHalf COLA below 100% funded (7% DR)",
  "misc2t_benCut2_colaCut2_highERC",      "Reduce benefit factor by 25% and \nHalf COLA below 100% funded (7% DR)",
  
  "misc2t_colaCut2lowerDR_lowERC",        "Half COLA below 100% funded (5% DR)",
  "misc2t_colaCut2lowerDR_highERC",       "Half COLA below 100% funded (5% DR)")
}
...
df_tbl_misc_a %>% 
 ...
 flextable() %>% 
  ...
  # Header
   ...
   add_header_lines(values = "Assumes investment returns of 7% every year (based on CalPERS assumptions)" ) %>%
   # add_header_lines(values = grpName_label) %>%  
   add_header_lines(values = "Estimated impacts on plan members \nof selected policy alternatives" ) %>%
   ...
   #hline(part = "body", i = c(1, 8),  border = officer::fp_border(color="gray40", width = 1.5)) %>% 
   hline(part = "body", i = c(2, 4), border = officer::fp_border(color="gray40", style = "dotted")) %>%  
   # vline(part = "body", j = c(4), border = officer::fp_border(color="gray40", style = "dotted"))
   #merge_v( j = "ERCpolicy") %>% 
   
  # Footer
  add_footer_lines(values = "Notes:") %>% 
  add_footer_lines(values = "- Benefit reductions are calculated based on state miscellaneous members. Results for state safety members are similar.") %>%
...
```{r}
# Impact on employer contributions, for draft report to Crane
...
{df_simNames3 <- 
  ...
  "all2t_benCut2_colaCut2_lowERC",       "Reduce benefit factor by 25% and \nHalf COLA below 100% funded (7% DR)",
  "all2t_benCut1_colaCut1_lowERC",       "Reduce benefit factor by 50% and \nNo COLA below 100% funded (7% DR)",
...
df_tbl_impactPlan %>% 
  ...
  # Header
   ...
   hline(part = "body", i = c(3, 5), border = officer::fp_border(color="gray40", style = "dotted")) %>%  
   # vline(part = "body", j = c(4), border = officer::fp_border(color="gray40", style = "dotted"))
   # merge_v( j = "ERCpolicy") %>% 
...
df <- bind_rows(ls_eg_sfty_benCut1$df_summary3 %>%
...
xlsx::write.xlsx2(df, paste0(here::here(), "/analysis/outputs_analysis/tbl_examples.xlsx"))
