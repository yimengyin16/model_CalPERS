...
# 1. Impact on active plan members
#  - Late-career active member: classic member at age 50 w/ entry age 25 in 2018, assuming retirement age of 55 
#  - Mid-career  active member: classic member at age 40 w/ entry age 25 in 2018, assuming retirement age of 55
...
# Mid-career classic member    
#   - Results do not depend on investment return scenarios and contribution policy. 

get_impactMembers <- function(valName_full,
                              valName_reduced,
                              fn_ea  = 25,
                              fn_age = 50,
                              fn_ageRet = 55,
...
# valName_full    = "misc2t_bf100_cola2"
# valName_reduced = "misc2t_bf50_cola2"
# fn_ea  = 25
# fn_age = 50
# fn_ageRet = 55
# dr = 0.07

df_val_full     <- readRDS(paste0(dir_outputs_val, paste0("val_", valName_full,   ".rds")))
df_val_reduced  <- readRDS(paste0(dir_outputs_val, paste0("val_", valName_reduced,".rds")))

df_B <- 
left_join(
  df_val_full$indivLiab$misc_classic$servRet.la %>% 
    filter(start_year == 2018 - (fn_age - fn_ea), ea == fn_ea, age_servRet ==  fn_ageRet) %>% 
     select(age_servRet, start_year, ea, age, year, B_full = B.servRet.la),
  
  df_val_reduced$indivLiab$misc_classic$servRet.la %>% 
    filter(start_year == 2018 - (fn_age - fn_ea), ea == fn_ea, age_servRet ==  fn_ageRet) %>% 
    select(age_servRet, start_year, ea, age, year, B_reduced = B.servRet.la) 
) %>% 
  mutate(B_pctChg = B_reduced / B_full - 1)

df_pvB <- 
  df_B %>% 
  filter(age <= 85) %>% 
  summarise(pvB_full    = sum(B_full    / (1 + dr)^(age - fn_ageRet)),
            pvB_reduced = sum(B_reduced / (1 + dr)^(age - fn_ageRet))
            ) %>% 
  mutate(pvB_pctChg = pvB_reduced / pvB_full - 1)

df <- list(df_B = df_B,
           df_pvB = df_B)
}

ls_activeAge50 <- get_impactMembers("misc2t_bf100_cola2",
                                    "misc2t_bf50_cola2",
                                    fn_ea = 25, 
                                    fn_age = 50)


ls_activeAge40 <- get_impactMembers("misc2t_bf100_cola2",
                                    "misc2t_bf50_cola2",
                                    fn_ea = 25, 
                                    fn_age = 40)

ls_activeAge50$df_pvB
ls_activeAge40$df_pvB
...
ls_benCut1 <- get_impactMembers2(simName          = "misc2t_benCut1_lowERC",
                                 valName_baseline = "misc2t_bf100_cola2")

ls_colaCut1 <- get_impactMembers2(simName          = "misc2t_colaCut1_lowERC",
                                  valName_baseline = "misc2t_bf100_cola2")

ls_benCut1_colaCut1 <- get_impactMembers2(simName          = "misc2t_benCut1_colaCut1_lowERC",
                                          valName_baseline  = "misc2t_bf100_cola2")


ls_benCut1$df_pvB    %>% kable()
ls_colaCut1$df_pvB   %>% kable()
ls_benCut1_colaCut1$df_pvB %>% kable()


ls_colaCut1$df_B %>% filter(start_year == 1973)
